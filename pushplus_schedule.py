...
# ================== 0. 环境初始化 ==================
# 强制北京时区，解决 GitHub Actions (UTC) 日期偏差
    ...
    try:
        ...
        if res.get("code") == 0:
            ...
            if h_type in [0, 3]:
                ...
                return False, res["type"]["name"], is_adj, target
            return True, res["type"]["name"], False, None
    except: pass
...
def main():
    ...
    natural_day = ["周一", "周二", "周三", "周四", "周五", "周六", "周日"][today.weekday()]
    
    # 3. 状态与颜色映射
    is_off, status_text, is_adj, target_day = get_holiday_status()
    query_day = target_day if (is_adj and target_day) else natural_day
    hitokoto = get_hitokoto()

    if is_adj:
        label, theme_color, theme_grad, header_title = "调休", "#FF4B2B", "#FF416C", "🚨 调休上课"
    elif is_off:
        label, theme_color, theme_grad, header_title = "放假", "#56AB2F", "#A8E063", "🏖️ 放假休息"
    else:
        label, theme_color, theme_grad, header_title = "上课", "#2193B0", "#6DDCFF", "📚 今日课表"
    ...
    if is_off and len(courses) == 0:
        ...
        push_title = f"{natural_day}课表 | {len(courses)}门课"
    ...
    if is_off and len(courses) == 0:
        ...
        if is_adj:
            course_cards += f"<div style='background:rgba(214,48,49,0.05); border:1px solid rgba(214,48,49,0.1); padding:8px; border-radius:10px; margin-bottom:15px; color:#d63031; font-size:12px; text-align:center;'>调休按 <b>{query_day}</b> 上课</div>"
        for c in courses:
            course_cards += f"""
    ...
    <div style='max-width: 400px; margin: 0 auto; background: #ffffff; border-radius: 24px; overflow: hidden; font-family: -apple-system, system-ui, sans-serif; border: 1px solid #f1f5f9; box-shadow: 0 10px 25px rgba(0,0,0,0.05);'>
        <!-- 页眉：垂直中心对齐 + 渐变 -->
        <div style='background: linear-gradient(135deg, {theme_color}, {theme_grad}); padding: 18px 22px; color: white;'>
                ...
                <tr>
                    <td align="left" valign="middle">
                        ...
                        <p style='margin: 0; opacity: 0.95; font-size: 13px; font-weight: 600; line-height: 1.1;'>
                            第 {curr_week} 周 · {natural_day} ({label})
                        </p>
                        {mini_bar}
                    </td>
